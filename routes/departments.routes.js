...
router.post('/departments', (req, res) => {
  req.db.collection('departments').insertOne({ name: req.body.name }, err => {
    if(err) res.status(500).json({ message: err });
...
router.put('/departments/:id', (req, res) => {
  req.db.collection('departments').updateOne({ _id: ObjectId(req.params.id) }, { $set: { name: req.body.name }}, err => {
    if(err) res.status(500).json({ message: err });
    else res.json({ message: 'OK' });
  });
});
